...
func (b Beacon) Activate(pos cube.Pos, clickedFace cube.Face, w *world.Tx, u item.User, ctx *item.UseContext) bool {
...
func (b Beacon) Tick(currentTick int64, pos cube.Pos, w *world.Tx) {
	if currentTick%80 == 0 {
		before := b.level
...
func (b Beacon) recalculateLevel(pos cube.Pos, w *world.Tx) int {
	var lvl int
	iter := 1
...
func (b Beacon) obstructed(pos cube.Pos, w *world.Tx) bool {
...
func (b Beacon) broadcastBeaconEffects(pos cube.Pos, w *world.Tx) {
	seconds := 9 + b.level*2
	if b.level == 4 {
		seconds--
	}
