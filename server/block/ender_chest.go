...
// UseOnBlock ...
func (c EnderChest) UseOnBlock(pos cube.Pos, face cube.Face, clickPos mgl64.Vec3, w *world.Tx, user item.User, ctx *item.UseContext) (used bool) {
...
// Activate ...
func (c EnderChest) Activate(pos cube.Pos, clickedFace cube.Face, w *world.Tx, u item.User, ctx *item.UseContext) bool {
...
// AddViewer ...
func (c EnderChest) AddViewer(w *world.Tx, pos cube.Pos) {
	if c.viewers.Inc() == 1 {
		c.open(w, pos)
	}
}
...
// RemoveViewer ...
func (c EnderChest) RemoveViewer(w *world.Tx, pos cube.Pos) {
	if c.viewers.Load() == 0 {
		return
	}
	if c.viewers.Dec() == 0 {
...
func (c EnderChest) open(w *world.Tx, pos cube.Pos) {
...
func (c EnderChest) close(w *world.Tx, pos cube.Pos) {
	for _, v := range w.Viewers(pos.Vec3()) {
