...
func (c Composter) BreakInfo() BreakInfo {
	return newBreakInfo(2, alwaysHarvestable, axeEffective, oneOf(c)).withBreakHandler(func(pos cube.Pos, w *world.Tx, u item.User) {
		if c.Level == 8 {
...
// Activate ...
func (c Composter) Activate(pos cube.Pos, clickedFace cube.Face, w *world.Tx, u item.User, ctx *item.UseContext) bool {
	if c.Level >= 7 {
		if c.Level == 8 {
			c.Level = 0
...
// ScheduledTick ...
func (c Composter) ScheduledTick(pos cube.Pos, w *world.Tx, r *rand.Rand) {
	if c.Level == 7 {
		c.Level = 8
		w.SetBlock(pos, c, nil)
